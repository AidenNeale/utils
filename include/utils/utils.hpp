...
namespace utils
{
	struct UtilParams
	{
		...
		UtilParams(const int &meanK = 9, const int &multi_thresh = 3,
			...
			: meanK_(meanK), multi_thresh_(multi_thresh),
			  outlier_radius_(outlier_radius), voxel_leaf_size_(voxel_leaf_size),
			  save_directory_(save_directory), mls_search_radius_(mls_search_radius),
			  mls_poly_order_(mls_poly_order), cropbox_min_(cropbox_min), cropbox_max_(cropbox_max)
		{
		}

		int meanK_, multi_thresh_, mls_poly_order_;
	...
	public:
		...
		void savePointcloud(const pcl::PointCloud<pcl::PointXYZI> &cloud, const std::string &save_directory);
		void savePointcloud(const pcl::PointCloud<pcl::PointXYZI> &cloud, const std::string &save_directory, const std::string &name);
		void savePointcloud(const pcl::PointCloud<pcl::PointNormal> &cloud, const std::string &save_directory, const std::string &name);
	...
	private:
		...
		void statisticalOutlierRemoval(const pcl::PointCloud<pcl::PointXYZI>::Ptr &cloud_in, pcl::PointCloud<pcl::PointXYZI>::Ptr &cloud_out);
		void voxelGridFilter(const pcl::PointCloud<pcl::PointXYZI>::Ptr &cloud_in, pcl::PointCloud<pcl::PointXYZI>::Ptr &cloud_out);
		void movingLeastSquares(const pcl::PointCloud<pcl::PointXYZI>::Ptr &cloud_in, pcl::PointCloud<pcl::PointXYZI>::Ptr &cloud_out);
		void distanceFilter(const pcl::PointCloud<pcl::PointXYZI>::Ptr &cloud_in, pcl::PointCloud<pcl::PointXYZI>::Ptr &cloud_out);
		void cropBoxFilter(const pcl::PointCloud<pcl::PointXYZI>::Ptr &cloud_in, pcl::PointCloud<pcl::PointXYZI>::Ptr &cloud_out);
	};
} // end namespace
